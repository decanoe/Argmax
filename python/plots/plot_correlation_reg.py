...
class PlotCorrelationReg(PlotCorrelation):
    ...
    def plot_single(self, algo: str, all_x: np.ndarray[float], all_y: np.ndarray[float]):
        ...
        p, = self.axis.plot(all_x, all_y, linestyle="", marker='o', color = run_info.algo_infos.get_color(), markersize=3, label=run_info.algo_infos.get_full_label(), alpha = min(1, 150 / len(all_x)))
        self.add_line(p)
        ...
        line, = self.axis.plot(x, model(x), label=run_info.algo_infos.get_full_label() + " regression", color = run_info.algo_infos.get_color(), linestyle = run_info.algo_infos.get_style())
        self.add_line(line, run_info.algo_infos.get_full_label() + " regression")
        #endregion