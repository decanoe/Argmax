...
class PlotOneRun(PlotAnnotedAxis):
    ...
    def plot(self):
        ...
        sorted_algos = sorted(self.window.get_selected_algos(), key=lambda algo:data_loader.get_file(self.window.get_n(), self.window.get_k(), algo).get_one_run_scores().fitness.max(), reverse=True)
