...
bool change_to_better_neighbor(std::unique_ptr<ReversibleInstance> &instance)
{
...
float Argmax::standard_derivation(std::vector<std::unique_ptr<ReversibleInstance>> &population)
{
...
float get_frequency_score(ReversibleInstance *instance, const std::unique_ptr<std::vector<std::map<float, unsigned int>>> &arg_frequency_map, unsigned int population_size) {
...
std::unique_ptr<ReversibleInstance> Argmax::evolution(std::function<std::unique_ptr<ReversibleInstance>()> spawner, evolution_parameters parameters, std::ostream *out)
{
    ...
    if (out)
    {
        ...
        *population_out << "generation\tage\tnb_args";
        ...
        std::unique_ptr<ReversibleInstance> temp = spawner();
    ...
    for (unsigned int g = 1; g < parameters.generation_count; g++)
    {
        ...
        if (p != progress_percent)
        {
            ...
            line_count++;
            for (size_t i = 0; i < std::min(10UL, population.size()); i++)
            {
