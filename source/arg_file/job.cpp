...
std::string Job::execute_job(bool debug) const {
    ...
    // get problem
    std::shared_ptr<Problem::Problem> problem = Problem::create_problem(get_instance_parameters(), parameters->get_seed(), debug);

    // create instance
    std::mt19937 rand = std::mt19937(parameters->get_seed());
    std::unique_ptr<ReversibleInstance> instance = FitnessInstance([&problem](const std::vector<bool>& a) -> float { return problem->evaluate(a); }, problem->get_nb_variables()).randomize_clone(rand);
